...
!>>> fourier hybf to htau, from matsubara frequency to imaginary time
  subroutine ctqmc_fourier_hybf(hybf,htau,atom)
     use constants
     use control
...
! external arguments
! hybridization function on imaginary time axis
     real(dp), intent(out) :: htau(ntime,norbs,norbs,natom)
...
! enforce hybridization function less than zero to ensure the causality
     do i=1,norbs
         do j=1,ntime
             if ( htau(j,i,i,atom) > zero ) htau(j,i,i,atom) = -eps6
         enddo ! over j={1,ntime} loop
     enddo ! over i={1,norbs} loop
