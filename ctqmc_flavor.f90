...
!>>> calculate the trace ratio for insert new segment or anti-segment on perturbation expansion series

  subroutine cat_insert_ztrace(flvr, anti, tau_start, tau_end, trace_ratio,atom)
...
! for segment case
     if ( anti .eqv. .false. ) then
         if ( tau_start < tau_end ) then
             ...
             call ctqmc_make_overlap(flvr, tau_start, tau_end, ovlp,atom)
! the new segment winds around the circle
         else
             dtau = beta - tau_start + tau_end - zero
             call ctqmc_make_overlap(flvr, zero, tau_end, ovlp1,atom)
             call ctqmc_make_overlap(flvr, tau_start, beta, ovlp2,atom)
             ovlp = ovlp1 + ovlp2
         endif
! for anti-segment case
         ...
         if ( tau_start > tau_end ) then
             ...
             call ctqmc_make_overlap(flvr, tau_end, tau_start, ovlp,atom)
! the new anti-segment winds around the circle
         else
             dtau = tau_start - zero + beta - tau_end
             call ctqmc_make_overlap(flvr, zero, tau_start, ovlp1,atom)
             call ctqmc_make_overlap(flvr, tau_end, beta, ovlp2,atom)
             ovlp = ovlp1 + ovlp2
         endif
...
! atom stuff. not sure.
     raux = dtau * ( - eimp(flvr,atom) )
     do i=1,norbs
         raux = raux - uumat(flvr, i,atom) * ovlp(i)
...
!>>> calculate the trace ratio for remove old segment or anti-segment on perturbation expansion series

  subroutine cat_remove_ztrace(flvr, anti, tau_start, tau_end, trace_ratio,atom)
...
!>>> calculate the trace ratio for left shift old segment or anti-segment on perturbation expansion series

  subroutine cat_lshift_ztrace(flvr, ring, tau_start1, tau_start2, trace_ratio,atom)
     use constants
     use control
     use context, only : uumat, eimp
...
! stretch the segment
         if ( tau_start1 > tau_start2 ) then
             ...
             call ctqmc_make_overlap(flvr, tau_start2, tau_start1, ovlp,atom)
! shrink the segment
         else
             dtau = tau_start2 - tau_start1
             call ctqmc_make_overlap(flvr, tau_start1, tau_start2, ovlp,atom)
         endif
...
! shrink the segment
         if ( tau_start1 > tau_start2 ) then
             ...
             call ctqmc_make_overlap(flvr, zero, tau_start2, ovlp1,atom)
             call ctqmc_make_overlap(flvr, tau_start1, beta, ovlp2,atom)
             ovlp = ovlp1 + ovlp2
! stretch the segment
         else
             dtau = tau_start1 - zero + beta - tau_start2
             call ctqmc_make_overlap(flvr, zero, tau_start1, ovlp1,atom)
             call ctqmc_make_overlap(flvr, tau_start2, beta, ovlp2,atom)
             ovlp = ovlp1 + ovlp2
         endif
...
!>>> calculate the trace ratio for right shift old segment or anti-segment on perturbation expansion series

  subroutine cat_rshift_ztrace(flvr, ring, tau_end1, tau_end2, trace_ratio,atom)
...
! shrink the segment
         if ( tau_end1 > tau_end2 ) then
             ...
             call ctqmc_make_overlap(flvr, tau_end2, tau_end1, ovlp,atom)
! stretch the segment
         else
             dtau = tau_end2 - tau_end1
             call ctqmc_make_overlap(flvr, tau_end1, tau_end2, ovlp,atom)
         endif
! it does wind around the circle
     else
! stretch the segment
         if ( tau_end1 > tau_end2 ) then
             ...
             call ctqmc_make_overlap(flvr, zero, tau_end2, ovlp1,atom)
             call ctqmc_make_overlap(flvr, tau_end1, beta, ovlp2,atom)
             ovlp = ovlp1 + ovlp2
! shrink the segment
         else
             dtau = tau_end1 - zero + beta - tau_end2
             call ctqmc_make_overlap(flvr, zero, tau_end1, ovlp1,atom)
             call ctqmc_make_overlap(flvr, tau_end2, beta, ovlp2,atom)
             ovlp = ovlp1 + ovlp2
         endif
     endif ! back if ( ring .eqv. .false. ) block
...
!>>> calculate the trace ratio for swap between segment and anti-segment on perturbation expansion series 

  subroutine cat_reswap_ztrace(flvr, trace_ratio,atom)
...
! swap to cstat = 3
     if      ( cstat == 0 ) then
         ts = zero
         te = beta
         call cat_insert_ztrace(flvr, .false., ts, te, p,atom)
         trace_ratio = p
...
! remove occupied states
         do i=1,ckink
             ...
             call cat_remove_ztrace(flvr, .false., ts, te, p,atom)
...
! shift index_s and index_e to create two empty rooms for as and ae
     ...
     enddo ! i={ckink,is,-1} loop

     do i=ckink,ie,-1
         index_e(i+1, flvr) = index_e(i, flvr)
     enddo ! i={ckink,ie,-1} loop
...
!>>> determine \tau_s, \tau_e and \tau_max for insert new segment or anti-segment
  ...
  subroutine ctqmc_make_flavor1(flvr, is, ie, anti, ladd, tau_start, tau_end, tau_max,atom)
...
! determine the position of tau_end and tau_max
         ...
         endif ! back if ( stts(flvr,atom) == 1 ) block
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

! case 3: there are segments, anti-segment configuration
!-------------------------------------------------------------------------
         if ( stts(flvr,atom) == 2 ) then
...
! determine the position of tau_end and tau_max
         ...
         endif ! back if ( stts(flvr,atom) == 2 ) block
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

! case 4: there is no segments, full configuration
!-------------------------------------------------------------------------
         if ( stts(flvr,atom) == 3 ) then
             ladd = .false.
             RETURN ! return to the parent subroutines immediately
         endif ! back if ( stts(flvr,atom) == 3 ) block
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
!-------------------------------------------------------------------------
         if ( stts(flvr,atom) == 0 ) then
             ladd = .false.
             RETURN ! return to the parent subroutines immediately
         endif ! if ( stts(flvr,atom) == 0 ) block
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
!>>> determine \tau_s, \tau_e and \tau_max for remove old segment or anti-segment
  ...
  subroutine ctqmc_make_flavor2(flvr, is, ie, anti, tau_start, tau_end, tau_max,atom)
...
!>>> determine \tau_s1 and \tau_s2 for lshift old segment or anti-segment
  ...
  subroutine ctqmc_make_flavor3(flvr, iso, isn, ring, tau_start1, tau_start2,atom)
...
!>>> determine \tau_e1 and \tau_e2 for rshift old segment or anti-segment
  ...
  subroutine ctqmc_make_flavor4(flvr, ieo, ien, ring, tau_end1, tau_end2,atom)
...
!>>> calculate the delta segment overlaps between current flavor channel and other flavor channels

  subroutine ctqmc_make_overlap(flvr, tau_start, tau_end, ovlp, atom)
...
! deal with the last segment (tailer)
             ts = time_s(index_s(rank(i,atom), i), i)
             te = beta
             call ctqmc_make_compare( tau_start, tau_end, ts, te, raux )
             ovlp(i) = ovlp(i) + raux
...
! case 4: full occupation
         else if ( stts(i,atom) == 3 ) then
             ovlp(i) = tau_end - tau_start

         endif ! back if ( stts(i,atom) == 0 ) block
...
! randomly, only used to debug the code
  ...
  subroutine ctqmc_make_segment(flvr, kink, anti,atom)
...
!>>> display segment information on the screen, only used to debug the code
  ...
  subroutine ctqmc_make_display(show_type,atom)
...
! apply normal display mode
     if ( show_type == 1 ) then
         do i=1,norbs
             ...
             write(mystd,'(4X,a,i4)') '# status:', stts(i,atom)

             write(mystd,'(4X,a,i4)') '# time_s data:', rank(i,atom)
             do j=1,rank(i,atom)
                 write(mystd,'(4X,2i4,f12.6)') i, j, time_s(index_s(j, i), i)
             enddo ! over j={1,rank(i,atom)} loop

             write(mystd,'(4X,a,i4)') '# time_e data:', rank(i,atom)
             do j=1,rank(i,atom)
...
! apply compat display mode
         ...
         do i=1,norbs
             ...
             write(mystd,'(4X,a,i4)') '# time_s and time_e data:', rank(i,atom)
             if      ( stts(i,atom) == 0 ) then
